...
def histogram(name, data, step=None, buckets=None, description=None):
  ...
  if isinstance(tf.distribute.get_strategy(), tf.distribute.experimental.TPUStrategy):
    return tf.tpu.outside_compilation(histogram_summary, data, buckets,
                                      summary_metadata, step)
